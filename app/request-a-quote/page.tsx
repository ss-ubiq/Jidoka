...
export default async function Page({ searchParams }: { searchParams: Promise<{ ref?: string; qty?: string; notes?: string; via?: string; quote?: string }> }) {
  const { ref, qty, notes, via, quote } = await searchParams;
  return (
    <FormPage
      mode="quote"
      ...
      reference={ref}
      quoteId={quote}
      prefill={{ quantity: qty, notes, via }}
    />
  );
}
